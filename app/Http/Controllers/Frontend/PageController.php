...
class PageController extends Controller
{
    ...
    {	
        ...
        if ($validator->fails()) {
            return redirect()->route('frontend.pages.contact')->withErrors($validator)->withInput();
        }
        ...
        $data = (object) array(
                ...
                'sending_as'     => $request->input('sending_as')
            );
		...
		Mail::to($setting->val, "New contact inquiry")->send(new sendContactInquiry($data));    
         ...
         return redirect()->route('frontend.pages.contact')->with('jsSuccess',"Your enquiry has been sent successfully.");
