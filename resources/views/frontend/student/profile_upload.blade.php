...
@php
		...
		$profileImage = asset($student->userData->profile_or_avatar_image);
						...
						@foreach ($avatars->chunk(16) as $chunk)
						 <div class="item">	
						  ...
						  @foreach ($chunk as $avatar)
							...
							<li>
								<a href="javascript:void(0)" class="profile_img avt-js {{($student->userData->avatar_id == $avatar->id ? 'active' : '')}}" id="avt-{{$avatar->id}}" onclick="profileUploadOb.setAvatar({{$avatar->id}})" title="">
